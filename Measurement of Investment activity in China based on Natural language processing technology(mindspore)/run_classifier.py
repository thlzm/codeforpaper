...
def do_eval(dataset=None, network=None, num_class=2, assessment_method="accuracy", load_checkpoint_path=""):
    ...
    for data in dataset.create_dict_iterator(num_epochs=1):
        ...
        first_token=model(input_ids, input_mask, token_type_id, label_ids)
        for j in range(input_ids.size()[0]):
            eval_all_result.append(first_token[j])
        #logits = model.predict(input_ids, input_mask, token_type_id, label_ids)
        #callback.update(logits, label_ids)
    print("==============================================================")
    #eval_result_print(assessment_method, callback)
    print("==============================================================")
    return eval_all_result
...
def run_classifier():
    ...
    if args_opt.do_eval.lower() == "true":
        ...
        eval_all_result=do_eval(ds, BertCLS, args_opt.num_class, assessment_method, load_finetune_checkpoint_path)
        output_test_file = os.path.join(args_opt.output_path, "test_results.csv")
        with open(output_test_file, "w") as writer:
            for element in eval_all_result:
                result_sample = element
                for ele in result_sample:
                    ele = ele.numpy().tolist()
