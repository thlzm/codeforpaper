...
def prompt_tune_train(train_valid_test_dataset_provider, model_provider,
...
def add_new_token(model):
    ...
    tokenizer.add_special_tokens({'seq_token': '[SQ]'})
    tokenizer.add_special_tokens({'eot_token': '[ET]'})
...
def backward_step(optimizer, model, loss, iteration):
    ...
    # 与optimizer.step要后一个batch做optimizer.zero_grad
    step_flag = (not args.gradient_accumulation_steps) or (iteration  % args.gradient_accumulation_steps == 0)
...
def train_step(forward_step_func, data_iterator, model, optimizer, lr_scheduler, iteration):
    ...
    loss, loss_reduced, target, predict = forward_step_func(data_iterator, model)

    if args.gradient_accumulation_steps:
        loss = loss / args.gradient_accumulation_steps

    # print_rank_0(f"target: {target.view(-1).tolist()}.")
    # print_rank_0(f"predict: {predict.view(-1).tolist()}.")
    # print_rank_0(f"loss_reduced: {loss_reduced}.")
    timers('forward').stop()
    ...
    step_flag = (not args.gradient_accumulation_steps) or ((iteration + 1) % args.gradient_accumulation_steps == 0)
    # Update parameters.
    ...
    if not (args.fp16 and optimizer.overflow):
        if step_flag: lr_scheduler.step()
    else:
        skipped_iter = 1
...
def evaluate(forward_step_func, data_iterator, model, verbose=False):
    ...
    with torch.no_grad():
        ...
        while iteration < args.eval_iters:
            ...
            # Forward evaluation.
            loss, loss_dict, target, predict = forward_step_func(data_iterator, model)

            labels_all.extend(target.view(-1).tolist())
            predicts_all.extend(predict.view(-1).tolist())
        ...
        acc_list = [torch.zeros_like(acc) for _ in range(mpu.get_data_parallel_world_size())]
        torch.distributed.all_gather(acc_list, acc, mpu.get_data_parallel_group())
        acc = torch.stack(acc_list, 0).view(-1).tolist()

        print_rank_0(f"Evaluate acc:\n {acc}")
        print_rank_0(f"Evaluate report:\n {report}")
        print_rank_0(f"Evaluate confusion:\n {confusion}")
...
def build_train_valid_test_data_iterators(
    ...
    if train_dataloader is not None:
        train_dataloader.batch_sampler.start_iter = args.iteration % len(train_dataloader)
        print_rank_0('setting training data start iteration to {}'.format(train_dataloader.batch_sampler.start_iter))
    if valid_dataloader is not None:
        ...
        print_rank_0('setting validation data start iteration to {}'.format(valid_dataloader.batch_sampler.start_iter))

    # Build iterators.
    if train_dataloader is not None:
        train_data_iterator = iter(train_dataloader)
        train_data_iterator = cycle(train_data_iterator)
    else:
        train_data_iterator = None

    if valid_dataloader is not None:
        valid_data_iterator = iter(valid_dataloader)
        valid_data_iterator = cycle(valid_data_iterator)
    else:
        valid_data_iterator = None

    if test_dataloader is not None:
        test_data_iterator = iter(test_dataloader)
        test_data_iterator = cycle(test_data_iterator)
    else:
