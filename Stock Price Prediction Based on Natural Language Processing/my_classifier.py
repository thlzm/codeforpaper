...
# limitations under the License.cfg.bert_network
# ============================================================================
...
# os.system('pip install https://ms-release.obs.cn-north-4.myhuaweicloud.com/1.0.1/MindSpore/gpu/ubuntu_x86/cuda-10.1/mindspore_gpu-1.0.1-cp37-cp37m-linux_x86_64.whl --trusted-host ms-release.obs.cn-north-4.myhuaweicloud.com')
# os.system('pip install https://ms-release.obs.cn-north-4.myhuaweicloud.com/1.0.1/MindSpore/ascend/euleros_aarch64/mindspore_ascend-1.0.1-cp37-cp37m-linux_aarch64.whl --trusted-host ms-release.obs.cn-north-4.myhuaweicloud.com')
# os.system('pip install https://ms-release.obs.cn-north-4.myhuaweicloud.com/1.0.1/MindSpore/ascend/ubuntu_x86/mindspore_ascend-1.0.1-cp37-cp37m-linux_x86_64.whl --trusted-host ms-release.obs.cn-north-4.myhuaweicloud.com')
...
parser.add_argument("--train_url", type=str, default=None,
                    help="训练平台的强制性参数,位置为: 版本号+V0001...")

# predict
parser.add_argument("--do_predict", type=str, default="false",
                    help="Schema path, it is better to use absolute path")

parser.add_argument("--predict_batch_size", type=int, default=1,
                    help="Predict batch size, default is 1")

parser.add_argument("--predict_data_file_path", type=str, default="",
                    help="Data path, it is better to use absolute path")

parser.add_argument("--predict_result_path", type=str, default="",
                    help="result path")

parser.add_argument("--predict_data_shuffle", type=str, default="false", choices=["true", "false"],
                    help="Enable predict data shuffle, default is true")
# 加载上述参数
...
# do_predict(ds, BertCLS, args_opt.num_class, assessment_method, load_finetune_checkpoint_path)


def do_predict(dataset=None, network=None, num_class=2, assessment_method="accuracy", load_checkpoint_path=""):
    """ do predict """
    if load_checkpoint_path == "":
        raise ValueError("Finetune model missed, predict task must load finetune model!")
    net_for_pretraining = network(bert_net_cfg, False, num_class)
    net_for_pretraining.set_train(False)
    param_dict = load_checkpoint(load_checkpoint_path)
    load_param_into_net(net_for_pretraining, param_dict)
    ...
    if os.path.exists(args_opt.predict_result_path):
        os.remove(args_opt.predict_result_path)
        print('移除已有结果')
    else:
        print('开始预测，冲！')
    ...
    for data in dataset.create_dict_iterator(num_epochs=1):
        ...
        input_ids, input_mask, token_type_id, label_ids = input_data
        logits = model.predict(input_ids, input_mask, token_type_id, label_ids)
        # logits应该就是输出的概率，可以重点参考下一步的update内部
        # 此处设置一个输出

        print(input_data[0][0])
        print(token_type_id[0])
        print(input_data[0][0]*token_type_id[0])
        print('='*50)
        with open(args_opt.predict_result_path ,'a') as f:
            # print(logits.asnumpy())
            for x in logits:
                x = x.asnumpy()
                # print(np.exp(x)/np.sum(np.exp(x)))
                f.write(str(np.exp(x)/np.sum(np.exp(x)))+'\n')
...
def run_classifier():
    ...
    if args_opt.do_train.lower() == "true":
        # 这个加载数据很关键——>要深入定义
        # 数据的columns包括["input_ids", "input_mask", "segment_ids", "label_ids"]
    ...
    if args_opt.do_eval.lower() == "true":
        ...
        # move_file_to_obs()
    # 自定义predict结果
    if args_opt.do_predict.lower() == "true":
        ds = create_classification_dataset(batch_size=args_opt.predict_batch_size, repeat_count=1,
                                           assessment_method=assessment_method,
                                           data_file_path=args_opt.predict_data_file_path,
                                           schema_file_path=args_opt.schema_file_path,
                                           do_shuffle=(args_opt.predict_data_shuffle.lower() == "true"))

        do_predict(ds, BertCLS, args_opt.num_class, assessment_method, load_finetune_checkpoint_path)
        # move_file_to_obs()

def move_file_to_local_dir():
    """将obs端的文件移动到工作目录本地"""
    local_path = os.path.join(os.getcwd(), 'data')  # /home/work/user-job-dir
    # local_path = os.getcwd()
    obs_path = 's3://nezha-mindspore/data'
    ...
    # obs下所有文件、文件夹列表
    obs_file_lst = mox.file.list_directory(obs_path)
    for i, file in enumerate(obs_file_lst):
        from_path = os.path.join(obs_path, file)
        to_path = os.path.join(local_path, file)
        mox.file.copy_parallel(from_path, to_path)
        print('File Move Sucess. Iter: %s. File: %s' % (i, file))
    print('Copy Success')
    print('-' * 30)
    print(local_path)
    print(os.listdir())
...
def move_file_to_obs():
    ...
    for i, file in enumerate(file_lst):
        from_path = os.path.join(local_path, file)
        to_path = os.path.join(obs_path, file)
        mox.file.copy_parallel(from_path, to_path)
        print('File Move Sucess. Iter: %s. File: %s' % (i, file))
