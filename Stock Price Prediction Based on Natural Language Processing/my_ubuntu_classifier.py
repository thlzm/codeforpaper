...
def do_predict(dataset=None, network=None, num_class=2, assessment_method="accuracy", load_checkpoint_path=""):
    ...
    for data in dataset.create_dict_iterator(num_epochs=1):
        ...
        softmax = np.exp(logits.asnumpy()) / np.exp(logits.asnumpy()).sum()
        softmaxs.append(softmax.flatten().tolist())
    # print(softmaxs)
    with open(args_opt.predict_result_path, 'w',  encoding='utf-8') as f:
        csv_writer = csv.writer(f)
        csv_writer.writerow(["prob0", "prob1"])
        csv_writer.writerows(softmaxs)
        print('result already printed')
...
def run_classifier():
    ...
    if args_opt.do_train.lower() == "false" and args_opt.do_eval.lower() == "false" and args_opt.do_predict.lower() == "false":
