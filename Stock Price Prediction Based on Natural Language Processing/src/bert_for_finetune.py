...
class BertFinetuneCell(nn.Cell):
    ...
    def __init__(self, network, optimizer, scale_update_cell=None):
        ...
        if scale_update_cell:
            self.loss_scale = Parameter(Tensor(scale_update_cell.get_loss_scale(), dtype=mstype.float32), name=None)
...
class BertSquad(nn.Cell):
    ...
    def __init__(self, config, is_training, num_labels=2, dropout_prob=0.0, use_one_hot_embeddings=False):
        ...
        self.is_training = is_training
        self.total_num = Parameter(Tensor([0], mstype.float32), name=None)
        self.start_num = Parameter(Tensor([0], mstype.float32), name=None)
        self.end_num = Parameter(Tensor([0], mstype.float32), name=None)
