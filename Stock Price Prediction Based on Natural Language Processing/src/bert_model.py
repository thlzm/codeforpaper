...
# EmbeddingLookup是生成词向量表然后得到输入的对应词向量
class EmbeddingLookup(nn.Cell):
    ...
    def __init__(self,
        ...
        self.embedding_table = Parameter(initializer(TruncatedNormal(initializer_range),
                                                     [vocab_size, embedding_size]), name=None)
...
# EmbeddingPostporcessor是为输入加上对应的A/B句子信息和位置信息
# 即EmbeddingPostporcessor输出的就是完整的输入(batch_size, seq_length, hidden_size)
    ...
    def __init__(self,
        ...
        self.embeddings_table = Parameter(
            initializer(TruncatedNormal(initializer_range),
                        [self.vocab_size, self.depth]), name=None)
...
# CreateAttentionMaskFromInputMask是为self-attention中计算value值时加上一些mask即控制某些位置不对value做贡献，
# 可以通过这个mask来控制每个词能看到的周围的词，默认是每个词可以看到句子中全部的词
...
class BertModel(nn.Cell):
    ...
    def construct(self, input_ids, token_type_ids, input_mask):
        ...
        # 将输入送入BertTransformer中得到输出，输出是一个tuple，保存了每一层的Encoder输出
        # 每一层的输出是(batch_size, seq_length, hidden_size)
        # bert encoder
        ...
        # 拿到最后一层输出的[CLS]位置对应的向量，在添加一层全连接进行语义空间转换
        # 用以计算NSP Loss (Next Sentence Prediction)
        # pooler
